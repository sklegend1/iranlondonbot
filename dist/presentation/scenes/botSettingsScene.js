...
exports.botSettingsScene = void 0;
...
const PrismaTargetGroupRepository_1 = require("../../infrastructure/db/repositories/PrismaTargetGroupRepository");
const settingRepo = new PrismaBotSettingRepository_1.PrismaBotSettingRepository();
const userRepo = new PrismaUserRepository_1.PrismaUserRepository();
const targetRepo = new PrismaTargetGroupRepository_1.PrismaTargetGroupRepository();
...
async function handleAddAdmin(ctx, text) {
...
async function handleGroupList(ctx, text, key) {
...
exports.botSettingsScene = new telegraf_1.Scenes.BaseScene("BOT_SETTINGS_SCENE");
exports.botSettingsScene.enter(async (ctx) => {
    await ctx.reply("⚙️ تنظیمات عمومی بات:", mainSettingsKeyboard());
});
exports.botSettingsScene.hears("🏠 بازگشت به منوی اصلی", async (ctx) => {
    await ctx.reply("بازگشت به منوی اصلی ✅");
    await ctx.scene.leave();
});
// ✅ افزودن ادمین
exports.botSettingsScene.hears("👑 افزودن ادمین", async (ctx) => {
    await ctx.reply("لطفاً آیدی عددی کاربر (telegramId) ادمین جدید را ارسال کن:");
    ctx.scene.state.waitingFor = "add_admin";
});
// ✅ لیست ادمین‌ها
exports.botSettingsScene.hears("📋 لیست ادمین‌ها", async (ctx) => {
...
// ✅ گروه‌های مرجع دعوت
exports.botSettingsScene.hears("🔗 گروه‌های مرجع دعوت", async (ctx) => {
...
exports.botSettingsScene.hears("💬 گروه‌های ما", async (ctx) => {
    ...
    if (!list.length) {
        ...
        await ctx.reply(`👥 لیست گروه‌های ما:\n${list.map((g) => `• ${g}`).join("\n")}`);
    await ctx.reply("لطفاً آی دی گروه خود را وارد کن (برای حذف بنویس حذف <نام>):");
    ctx.scene.state.waitingFor = "our_groups";
});
// ✅ کانال اصلی
exports.botSettingsScene.hears("📢 کانال اصلی", async (ctx) => {
    await ctx.reply("لطفاً یوزرنیم یا لینک کانال اصلی را ارسال کن:");
    ctx.scene.state.waitingFor = "main_channel";
});
// ✅ پیام‌های خودکار
exports.botSettingsScene.hears("💬 پیام‌های خودکار", async (ctx) => {
    await ctx.reply("چه پیامی را می‌خواهی تنظیم کنی؟", telegraf_1.Markup.keyboard([
...
exports.botSettingsScene.hears("⚙️ وضعیت ربات مدیریت گروه", async (ctx) => {
    const status = await settingRepo.getValue("group_bot_enabled");
    const isEnabled = status?.value === "true";
    const toggleKeyboard = telegraf_1.Markup.keyboard([
...
// ✅ هندل پیام‌های ورودی
exports.botSettingsScene.on("text", async (ctx) => {
