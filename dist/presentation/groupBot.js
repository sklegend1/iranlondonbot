...
exports.setupGroupBot = setupGroupBot;
...
function setupGroupBot(groupBot) {
    ...
    groupBot.on("new_chat_members", async (ctx) => {
        ...
        const welcomeText = (await prisma.botSetting.findUnique({ where: { key: "welcome_message" } }))?.value ??
    ...
    groupBot.on("message", async (ctx, next) => {
        if (ctx.state && ctx.state.botMode !== "group")
            return next();
    ...
    groupBot.on("message", async (ctx, next) => {
        ...
        try {
            ...
            if (member.status === "left" || member.status === "kicked") {
                await ctx.deleteMessage();
                await ctx.reply("⛔️ برای فعالیت در گروه باید عضو کانال باشی.", telegraf_1.Markup.inlineKeyboard([telegraf_1.Markup.button.url("📢 عضویت در کانال", channelLink)]));
                return;
            }
        }
        catch (err) {
