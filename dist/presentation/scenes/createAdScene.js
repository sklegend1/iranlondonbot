...
const botSettingRepo = new PrismaBotSettingRepository_1.PrismaBotSettingRepository();
function mainMenuKeyboard() {
    return telegraf_1.Markup.keyboard([["🏠 بازگشت به منو"]]).resize();
}
function categoriesKeyboard(categories) {
    const rows = categories.map(c => [c.name]);
    rows.push(["🏠 بازگشت به منو"]);
    return telegraf_1.Markup.keyboard(rows).resize();
}
exports.createAdScene = new telegraf_1.Scenes.WizardScene("CREATE_AD_SCENE", 
// ─────────────── مرحله ۱: انتخاب دسته‌بندی
async (ctx) => {
    const cats = await catRepo.findAll();
    ctx.wizard.state.categories = cats;
    const validCategories = cats.filter((cat) => cat.id !== undefined);
...
async (ctx) => {
    ...
    const cats = ctx.wizard.state.categories;
    const cat = cats.find(c => c.name === text);
    if (!cat)
        return ctx.reply("❌ گزینه نامعتبر است. لطفاً از لیست انتخاب کن.");
...
async (ctx) => {
    ...
    await ctx.reply("⏰ لطفاً زمان انتشار را انتخاب کنید:", telegraf_1.Markup.keyboard([
...
async (ctx) => {
    ...
    if (text === "📤 انتشار فوری") {
        ...
        const { categoryPrice } = ctx.wizard.state;
        await ctx.reply(`💳 مبلغ پرداخت برای این دسته‌بندی: *${categoryPrice} *\n\n ${msg?.value} \n\n رسید پرداخت (عکس یا متن) را ارسال نمایید`, { parse_mode: "Markdown", ...mainMenuKeyboard() });
...
async (ctx) => {
    ...
    ctx.wizard.state.startAt = startAt;
    ctx.wizard.state.endAt = endAt;
    await ctx.reply(`💳 مبلغ پرداخت برای این دسته‌بندی: *${categoryPrice} تومان*\n\nلطفاً مبلغ را به شماره کارت زیر واریز کنید و سپس رسید پرداخت (عکس یا متن) را ارسال نمایید.\n\n🏦 1234-5678-9012-3456 به نام "ربات تبلیغات"`, { parse_mode: "Markdown", ...mainMenuKeyboard() });
    return ctx.wizard.next();
}, 
...
async (ctx) => {
    ...
    const { categoryId, content, fileId, startAt, endAt } = ctx.wizard.state;
