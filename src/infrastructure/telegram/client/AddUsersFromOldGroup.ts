...
import { PrismaOperatorRepository } from "../../db/repositories/PrismaOperatorRepository";
...
import { PrismaInviteLogRepository } from "../../db/repositories/PrismaInviteLogRepository";
import bigInt from "big-integer";
import { PrismaTargetGroupMemberRepository } from "../../db/repositories/PrismaTargetGroupMemberRepository";

const opRepo = new PrismaOperatorRepository();
const userRepo = new PrismaScrapedUserRepository();
const ahRepo = new PrismaAccessHashRepository();
const logRepo = new PrismaInviteLogRepository();
const targetMemberRepo = new PrismaTargetGroupMemberRepository();
 ...
 */
async function resolveEntityAndSave(client: any, userIdentifier: string | number) {
  try {
    const ent: any = await client.getEntity(userIdentifier as any);
...
async function safeInviteWithOperator(client: any, targetEntity: any, userRow: any, operatorId: number) {
  // Try by username first
  if (userRow.username) {
    try {
      const uname = userRow.username.startsWith("@") ? userRow.username : `@${userRow.username}`;
      await client.invoke(new Api.channels.InviteToChannel({ channel: targetEntity, users: [uname] }));
  ...
  if (uidBig && ahBig) {
    try {
      await client.invoke(new Api.channels.InviteToChannel({
        ...
        users: [ new Api.InputUser({ userId: bigInt(uidBig), accessHash: bigInt(ahBig) }) ],
      }));
      ...
      if (/INVALID_ID|USER_ID_INVALID|HASH_INVALID|INVALID_PEER/.test(msg)) {
        ...
        if (resolved.accessHash) {
          ...
          try {
            await client.invoke(new Api.channels.InviteToChannel({
              channel: targetEntity,
              users: [ new Api.InputUser({ userId: bigInt(uidBig), accessHash: bigInt(ah2!) }) ],
            }));
  ...
  try {
    const exportRes: any = await client.invoke(new Api.messages.ExportChatInvite({ peer: targetEntity }));
 ...
 */
export async function addUsersFromDbToGroup(groupKey: string, targetGroupIdentifier: string, options?: { batchSize?: number }) {
  const apiId = Number(process.env.TELEGRAM_API_ID);
  const apiHash = process.env.TELEGRAM_API_HASH!;
  const batchSize = options?.batchSize ?? 20;

  const operators = await opRepo.getActiveOperators();
  if (!operators.length) throw new Error("No active operators found");
  ...
  if (!aliveOperators.length) throw new Error("No operator clients available");
  ...
  if (!toProcess.length) {
    ...
    // disconnect clients
    for (const c of clientsMap.values()) await c.disconnect();
    return;
  }
