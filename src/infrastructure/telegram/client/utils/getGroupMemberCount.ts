 ...
 */
export async function getGroupMemberCount(client: TelegramClient, entity: any): Promise<number> {
    try {
      // Try channel full info first (works for channels & supergroups)
      const res: any = await client.invoke(new Api.channels.GetFullChannel({ channel: entity }));
      const total =
        ...
        0;
      if (total && total > 0) return Number(total);
    } catch (err) {
      // ignore and try next
    ...
    try {
      ...
      const res2: any = await client.invoke(new Api.messages.GetFullChat({ chatId: entity }));
      const total2 =
        (res2?.full_chat && (res2.full_chat.participants_count ?? res2.full_chat.participantsCount)) ||
        (res2?.fullChat && (res2.fullChat.participantsCount ?? res2.fullChat.participants_count)) ||
        0;
      if (total2 && total2 > 0) return Number(total2);
    } catch (err) {
      // ignore
