...
exports.addGroupUsersToDB = addGroupUsersToDB;
const telegram_1 = require("telegram");
const sessions_1 = require("telegram/sessions");
const tl_1 = require("telegram/tl");
const client_1 = require("@prisma/client");
const fetchMembersWithFallback_1 = require("./utils/fetchMembersWithFallback");
...
const accessHashRepo = new PrismaAccessHashRepository_1.PrismaAccessHashRepository();
function sleep(ms) {
 ...
 */
function sampleExponentialMean(mean, min, max) {
    const u = Math.random();
 ...
 */
async function countAddsLast24Hours(operatorId) {
 ...
 */
function buildInviteTargetForUser(user) {
    if (user.username)
    ...
    return new tl_1.Api.InputUser({
        userId: (0, ToBigIntOrLong_1.toBigIntOrLong)(userId),
        accessHash: (0, ToBigIntOrLong_1.toBigIntOrLong)(accessHash),
    });
}
// --------- Main ----------
async function addGroupUsersToDB(apiId, apiHash) {
    if (!apiId || !apiHash) {
    ...
    const oldUsers = await (0, fetchMembersWithFallback_1.fetchMembersWithFallback)(client, oldEntity); // fetch up to 10k messages
