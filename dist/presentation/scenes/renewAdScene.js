...
exports.renewAdScene = new telegraf_1.Scenes.WizardScene("RENEW_AD_SCENE", 
...
async (ctx) => {
    ...
    const days = 30;
    if (!days)
        return ctx.reply("❌ لطفاً مدت معتبر انتخاب کن.");
    const addDays = 30;
    const ad = await adRepo.findById(adId);
    if (!ad)
        return ctx.reply("❌ آگهی یافت نشد.");
